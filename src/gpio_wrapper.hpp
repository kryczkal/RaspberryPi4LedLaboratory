...
private:
  ...
  void handleError(const std::string &operation) const {
    if (handle_) {
      std::cerr << "Error in " << operation << ": " << gpio_errmsg(handle_)
                << " (errno " << gpio_errno(handle_) << ")" << std::endl;
    } else {
      std::cerr << operation << " failed: Invalid GPIO handle" << std::endl;
    }

    throw std::runtime_error(
        operation +
        " failed: " + (handle_ ? gpio_errmsg(handle_) : "Invalid handle"));
  }
...
public:
  GpioWrapper() : handle_(gpio_new()) {
    if (!handle_) {
      throw std::runtime_error(
          "gpio_new() failed: Could not allocate GPIO handle.");
    }
  }
  ...
  bool open(const std::string &chip_path, unsigned int line,
            gpio_direction_t direction) {
    if (opened_) {
      std::cerr << "GPIO line " << line_ << " already opened.\n";
      return false;
    }
  ...
  bool open_advanced(const std::string &chip_path, unsigned int line,
  ...
  bool set_edge(gpio_edge_t edge) {
    ...
    if (gpio_set_edge(handle_, edge) < 0) {
      ...
      if (gpio_get_direction(handle_, &current_dir) >= 0 &&
          current_dir != GPIO_DIR_IN) {
        std::cerr << "Cannot set edge on non-input GPIO line " << line_
                  << ".\n";
        return false;
      }
  ...
  void close() {
    if (opened_ && handle_) {
      if (gpio_close(handle_) < 0) {
        std::cerr << "Error closing GPIO line " << line_ << ": "
                  << gpio_errmsg(handle_) << " (errno " << gpio_errno(handle_)
                  << ")" << std::endl;
      }
      opened_ = false;
    }
