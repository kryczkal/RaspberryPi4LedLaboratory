...
LedController::LedController(const std::vector<unsigned int> &led_pins,
                             const std::string &chip_path)
    : chip_path_(chip_path), current_pattern_index_(0),
      animation_delay_ms_(250), running_(false) {
  if (led_pins.empty()) {
  ...
  try {
    for (unsigned int pin : led_pins) {
      ...
      if (!led.open(chip_path_, pin, GPIO_DIR_OUT)) {
        std::cerr << "Failed to open LED GPIO " << pin << " on " << chip_path_
                  << std::endl;
    ...
    std::cerr << "Exception during LedController initialization: " << e.what()
              << std::endl;
    throw;
  }

  std::cout << "LedController initialized with " << leds_.size() << " LEDs."
            << std::endl;
}
...
void LedController::addPattern(const LedPattern &pattern) {
  ...
  if (!pattern.empty() && !pattern[0].empty() &&
    ...
    std::cerr << "Error adding pattern: Pattern step size ("
              << pattern[0].size() << ") does not match number of LEDs ("
              << leds_.size() << ")." << std::endl;
    return;
  }
  patterns_.push_back(pattern);
  std::cout << "Added pattern. Total patterns: " << patterns_.size()
            << std::endl;
}
...
void LedController::nextPattern() {
  ...
  std::cout << "Switched to next pattern: " << current_pattern_index_.load()
            << std::endl;
}
...
void LedController::previousPattern() {
  ...
  current_pattern_index_ =
      (current_pattern_index_ - 1 + patterns_.size()) % patterns_.size();
  std::cout << "Switched to previous pattern: " << current_pattern_index_.load()
            << std::endl;
}
...
void LedController::increaseSpeed(int delta_ms) {
  ...
  std::cout << "Animation speed increased. New delay: " << new_delay << " ms"
            << std::endl;
}
...
void LedController::decreaseSpeed(int delta_ms) {
  ...
  animation_delay_ms_.store(new_delay);
  std::cout << "Animation speed decreased. New delay: " << new_delay << " ms"
            << std::endl;
}
  ...
  while (running_.load()) {
    ...
    for (const auto &step : current_pattern) {
      ...
      if (step.size() == leds_.size()) {
        ...
        fprintf(
            stderr,
            "Pattern %d step size mismatch (%zu vs %zu LEDs). Skipping step.\n",
            pattern_idx, step.size(), leds_.size());
      }

      std::this_thread::sleep_for(
          std::chrono::milliseconds(animation_delay_ms_.load()));
    }
  }
  turnOffAllLeds();
}
